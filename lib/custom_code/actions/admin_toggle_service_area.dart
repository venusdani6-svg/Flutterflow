...
Future<dynamic> adminToggleServiceArea(
    String? prefecture, bool? newActive) async {
  try {
    final targetPrefecture = prefecture ?? '';
    final targetActive = newActive ?? false;
    ...
    final current = currentRaw is Map
        ...
        : <String, dynamic>{};
    bool active(String key) => current[key] == true;
    ...
    final newAreas = order.entries.map((entry) {
      final isActive =
          entry.key == targetPrefecture ? targetActive : active(entry.value);
      return {'prefecture': entry.key, 'active': isActive};
    }).toList();
